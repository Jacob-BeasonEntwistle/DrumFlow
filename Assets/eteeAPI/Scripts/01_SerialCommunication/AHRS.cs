/*
 * 
 * Update void is not called, only the following are used:
 * 
 * 
 */
...
namespace AHRS
{
    public class MadgwickAHRS
    {
        ...
        public void UpdateAbsolute(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
        {
        ...
        /// Total sqrt: 3
        /// </remarks>
        /// NOTE: this function has shaky or jittery hands, but it drifts less. This is the function which is currently used
        /// and the jittering / shaky hands was addressed by smoothening the rotation animation with linear interpolation.
        /// For smoothening animation, see script: eteeDevice.cs
        public void UpdateRelative(float gx, float gy, float gz, float ax, float ay, float az)
        {
            float q1 = Quaternion[0], q2 = Quaternion[1], q3 = Quaternion[2], q4 = Quaternion[3];   // short name local variable for readability
            float norm;
            float s1, s2, s3, s4;
